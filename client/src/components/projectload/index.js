            ...
            <div className='project-text-container'>
                ...
                <div className='project-info-container'>
                    <div ref={descRef} className={`project-desc ${descVisible ? 'animate-desc' : ''}`}>
                        ...
                        <p className={`desc-text ${descVisible ? 'animate-text' : ''}`}>{projectData.desc}</p>
                    </div>
                    <div ref={detRef} className={`project-details ${detVisible ? 'animate-desc' : ''}`}>
                        <h3 className='details-title'>Details</h3>
                        <ul>
                            <li className={`${detVisible ? 'animate-text' : ''}`}><strong>Type </strong>{projectData.type}</li>
                            <li className={`${detVisible ? 'animate-text' : ''}`}><strong>Tools </strong>{projectData.tools}</li>
                                {projectData.contribution && <li className={`${detVisible ? 'animate-text' : ''}`}><strong>Contributions </strong>{projectData.contribution}</li>}
                        </ul>
                    </div>
                </div>
            </div>
